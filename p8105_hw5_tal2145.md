...
sim_results %>% 
  ...
  geom_col() +
  labs(
    title = "True Value of β1 to the Mean Estimated β1 Among Samples Where the Null was Rejected",
    x = "True β1",
    y = "Estimated β1"
  ) 
```
...
likely to correctly reject the null hypothesis. At higher effect sizes,
our power is increased, so we see a better approximation of β1.
